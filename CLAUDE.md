...
- **Sessions rotate on idle, not per process.** Long-lived MCP clients (Cursor) keep one tap alive for days. When a substantive (non-ping/list) request arrives after ≥30 min of quiet (`HORATIO_ROTATE_IDLE_MS`, ≤0 disables), the tap ends the current session (`session_end` meta, `endReason: 'rotated'`) and opens a fresh one with its own follower. Rotation happens only at request boundaries (req/res pairs never straddle sessions), lives on the logging path, and is fully try/caught — failure means recording continues in the old session.
